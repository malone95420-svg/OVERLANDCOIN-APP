...
export function QuestCards({ quests }: { quests: Quest[] }) {
  ...
  const goToMyLocation = useCallback(() => {
    if (userGeo.status === "denied") {
      // Still bump so FlyToUser can attempt getCurrentPosition / surface OS prompt again
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-[#0a121c]">
      ...
      <div className="pointer-events-none absolute inset-0 z-[1100]">
        ...
        {/* Live weather for GPS — under search badges on the left */}
        <div className="pointer-events-auto absolute left-3 top-[9.5rem] z-10 md:left-auto md:right-[11.5rem] md:top-3">
          <LiveWeatherChip geo={userGeo} />
        </div>

        {/* Always-visible My location FAB (not buried in quest detail) */}
        <div className="pointer-events-auto absolute right-3 top-14 z-10 flex flex-col items-end gap-1 sm:top-[3.75rem] md:top-16">
          {locationDenied && (
          ...
          <button
            ...
            className={`flex h-11 items-center gap-2 rounded-xl border px-3 text-xs font-semibold shadow-lg backdrop-blur-md transition ${
              locationActive
                ? "border-cyan-accent/50 bg-cyan-accent/20 text-cyan-accent"
        ...
        {listOpen && (
          <div className="pointer-events-auto absolute inset-x-3 bottom-14 top-28 z-20 flex w-auto flex-col overflow-hidden rounded-2xl border border-white/15 bg-black/90 shadow-2xl backdrop-blur-md sm:inset-x-auto sm:right-3 sm:top-16 sm:w-[min(100%-1.5rem,20rem)] md:top-14">
